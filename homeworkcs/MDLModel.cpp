...
//	glCullFace(GL_FRONT);
	...
	//	printf("mesh %d  %d\n", Loop, Texture[SkinReference[MeshPtr->SkinReference]].Index);
		...
		while (NumTriangles = *(Triangles++))
		{
			...
			for (; NumTriangles > 0; NumTriangles--, Triangles += 4)
			{
				...
				//printf("tex %f %f\n", Triangles[2] * U, Triangles[3] * V);
				//	printf("tex %f %f %f %f %f\n", Triangles[2] * U, Triangles[3] * V, LightValue[0], LightValue[1],LightValue[2]);
...
{	long	Row1[256];
	...
	if (glIsTexture((GLuint)CurrentTexture))
	{
		printf("ok\n");
	}
	else
